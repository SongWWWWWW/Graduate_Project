      ...
      <div class="my_r chapter-write-page" :class="{ 'fullscreen-mode': isFullscreen }">
        ...
        <div class="write-header">
          ...
          <div class="header-actions">
            ...
            />
            <el-button type="primary" size="large" @click="saveChapter" :loading="saving">
        ...
        <div class="chapter-title-row">
          ...
          <span class="word-count">{{ chapter.chapterContent.length }} 字</span>
        </div>
            ...
            <div class="ai-chat-area" ref="aiChatRef">
              ...
              >
                ...
                <div v-if="msg.role === 'assistant'" class="ai-message-actions">
                  ...
                  <el-link
                    v-if="msg.action === 'continue' || msg.action === 'chat'"
                    type="primary"
                    :underline="false"
                  ...
                  <el-link
                    v-if="msg.action === 'expand' || msg.action === 'condense' || msg.action === 'polish'"
                    type="primary"
                    :underline="false"
                    @click="replaceSelection(msg.content)"
                  >
...
import { publishChapter, aiStreamChat } from "@/api/author";
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      bookId: route.query.bookId || route.query.id,
    ...
    // 自动保存相关
    const saveDraft = () => {
      const key = `chapter_draft_${state.bookId}`;
    ...
    const loadDraft = () => {
      ...
      if (draft) {
        try {
          ...
          if (parsed.chapterName || parsed.chapterContent) {
            state.chapter.chapterName = parsed.chapterName || "";
            state.chapter.chapterContent = parsed.chapterContent || "";
            state.chapter.isVip = parsed.isVip || 0;
    ...
    const quickAI = async (action) => {
      ...
      try {
        ...
        state.aiMessages.push({ role: "assistant", content: state.streamingText, action });
      } catch (err) {
        if (err.name === "AbortError") {
          // 用户主动取消，无需提示
        } else if (err.message && (err.message.includes("登录") || err.message.includes("过期") || err.message.includes("认证") || err.message.includes("A0230"))) {
    ...
    // 解析 SSE 数据中的文本内容（支持纯文本和 JSON 格式）
    const parseSSEData = (raw) => {
      if (raw === "[DONE]") return { done: true, text: "" };
      // 尝试解析 JSON 格式的 SSE 数据（如 {"content":"xxx"}）
      try {
        const json = JSON.parse(raw);
        if (typeof json.content === "string") return { done: false, text: json.content };
        if (typeof json.message === "string") return { done: false, text: json.message };
        if (typeof json.data === "string") return { done: false, text: json.data };
        if (typeof json.text === "string") return { done: false, text: json.text };
      } catch (e) {
        // 不是 JSON，按纯文本处理
      }
    ...
    const handleStream = async (response) => {
      ...
      if (!contentType.includes("text/event-stream")) {
        ...
        try {
          ...
          if (e.message && (e.message.includes("服务") || e.message.includes("登录") || e.message.includes("过期") || e.message.includes("认证") || e.message.includes("权限"))) {
            throw e;
          }
        }
      ...
      let done = false;
      let buffer = ""; // 缓存不完整的 SSE 行

      // 带超时的读取，防止后端挂起导致无限等待
      const readWithTimeout = (ms = 60000) => {
        return Promise.race([
          reader.read(),
      ...
      while (!done) {
        const { value, done: streamDone } = await readWithTimeout();
        done = streamDone;
        if (value) {
          ...
          for (const line of lines) {
            ...
            if (trimmed.startsWith("data:")) {
              ...
              const { done: isDone, text } = parseSSEData(raw);
              if (isDone) {
                done = true;
              } else if (text) {
    ...
    const sendAI = async () => {
      ...
      try {
        ...
        await handleStream(response);
        state.aiMessages.push({ role: "assistant", content: state.streamingText, action: 'chat' });
      } catch (err) {
        if (err.name === "AbortError") {
          // 用户主动取消，无需提示
        } else if (err.message && (err.message.includes("登录") || err.message.includes("过期") || err.message.includes("认证") || err.message.includes("A0230"))) {
