...
public class BookServiceImpl implements BookService {
    ...
    private BookVectorService bookVectorService;

    private static final String BOOK_VISIT_REDIS_KEY_PREFIX = "book:visit:";

    private static final Duration BOOK_VISIT_EXPIRE = Duration.ofHours(24);
    ...
    @Override
    public RestResp<Void> addVisitCount(Long bookId, String clientIp) {
        if (clientIp == null || clientIp.isBlank()) {
            clientIp = "unknown";
        }
        // IPv6 地址中的冒号替换，避免与 Redis key 分隔符混淆
        String safeIp = clientIp.replace(":", "-");
        String key = BOOK_VISIT_REDIS_KEY_PREFIX + bookId + ":" + safeIp;
        Boolean firstVisit = stringRedisTemplate.opsForValue()
            .setIfAbsent(key, "1", BOOK_VISIT_EXPIRE);
    ...
    @ContentAudit(value = ContentAudit.AuditBizType.COMMENT, textExpr = "#dto.commentContent")
    @Lock(prefix = "userComment")
    @Override
    public RestResp<Void> saveComment(
        @Key(expr = "#{userId + '::' + bookId + '::' + chapterId}") UserCommentReqDto dto) {
        // 校验书籍是否存在
        BookInfo bookInfo = bookInfoMapper.selectById(dto.getBookId());
        if (bookInfo == null) {
            return RestResp.fail(ErrorCodeEnum.BOOK_NOT_FOUND);
        }
        Long chapterId = dto.getChapterId() != null ? dto.getChapterId() : 0L;
        // 校验用户是否已对该章节发表评论
    ...
    @Override
    public RestResp<BookCommentRespDto> listNewestComments(Long bookId, Long chapterId) {
        Long cid = chapterId != null ? chapterId : 0L;
        // 查询评论总数
    ...
    @ContentAudit(value = ContentAudit.AuditBizType.BOOK_INFO, textExpr = "#dto.bookDesc + #dto.bookName")
    @Override
    ...
    @ContentAudit(value = ContentAudit.AuditBizType.BOOK_CHAPTER, textExpr = "#dto.chapterContent", bizIdExpr = "#dto.bookId")
    ...
    public RestResp<Void> saveBookChapter(ChapterAddReqDto dto) {
        ...
        //  d) 异步向量化入库（如果 Milvus 已启用）
        if (bookVectorService != null) {
            bookVectorService.indexChapter(dto.getBookId(), newBookChapter.getId(), dto.getChapterContent());
        }
        return RestResp.ok();
    }
    ...
    public RestResp<Void> deleteBookChapter(Long chapterId) {
        ...
        // 10.删除向量数据（如果 Milvus 已启用）
        if (bookVectorService != null) {
            bookVectorService.deleteChapterVector(chapterId);
        }
        return RestResp.ok();
    }
    ...
    @ContentAudit(value = ContentAudit.AuditBizType.BOOK_CHAPTER, textExpr = "#dto.chapterContent", bizIdExpr = "#chapterId")
    @Transactional
    @Override
    public RestResp<Void> updateBookChapter(Long chapterId, ChapterUpdateReqDto dto) {
        ...
        // 9.发送小说信息更新的 MQ 消息
        amqpMsgManager.sendBookChangeMsg(chapter.getBookId());
        // 10.更新向量数据（如果 Milvus 已启用）
        if (bookVectorService != null) {
            bookVectorService.indexChapter(chapter.getBookId(), chapterId, dto.getChapterContent());
        }
        return RestResp.ok();
    }
