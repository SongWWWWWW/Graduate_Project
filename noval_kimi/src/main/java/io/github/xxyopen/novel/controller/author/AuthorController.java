...
public class AuthorController {
     ...
     * 作家工作台数据查询接口
     */
    @Operation(summary = "作家工作台数据查询接口")
    @GetMapping("dashboard")
    public RestResp<AuthorDashboardRespDto> getDashboard() {
        return authorService.getDashboard();
    }

    /**
     * 作家作品统计列表查询接口
     */
    @Operation(summary = "作家作品统计列表查询接口")
    @GetMapping("book/stats")
    public RestResp<PageRespDto<AuthorBookStatsRespDto>> listBookStats(@ParameterObject PageReqDto dto) {
        return authorService.listBookStats(dto);
    }

    /**
     * 作家稿费收入列表查询接口
     */
    @Operation(summary = "作家稿费收入列表查询接口")
    @GetMapping("income")
    public RestResp<PageRespDto<AuthorIncomeRespDto>> listIncome(@ParameterObject PageReqDto dto) {
        return authorService.listIncome(dto);
    }

    /**
     * 作家稿费收入明细列表查询接口
     */
    @Operation(summary = "作家稿费收入明细列表查询接口")
    @GetMapping("income/detail")
    public RestResp<PageRespDto<AuthorIncomeDetailRespDto>> listIncomeDetail(
        @Parameter(description = "小说ID") @RequestParam(name = "bookId", required = false) Long bookId,
        @ParameterObject PageReqDto dto) {
        return authorService.listIncomeDetail(bookId, dto);
    }

    /**
     * 作家作品评论列表查询接口
     */
    @Operation(summary = "作家作品评论列表查询接口")
    @GetMapping("book/comments")
    public RestResp<PageRespDto<AuthorBookCommentRespDto>> listBookComments(
        @Parameter(description = "小说ID") @RequestParam(name = "bookId", required = false) Long bookId,
        @ParameterObject PageReqDto dto) {
        return authorService.listBookComments(bookId, dto);
    }

    /**
     * 作家资料查询接口
     */
    @Operation(summary = "作家资料查询接口")
    @GetMapping("profile")
     ...
     * 作家资料更新接口
     */
    @Operation(summary = "作家资料更新接口")
    @PutMapping("profile")
    public RestResp<Void> updateProfile(@Valid @RequestBody AuthorProfileUpdateReqDto dto) {
        return authorService.updateProfile(dto);
    }
